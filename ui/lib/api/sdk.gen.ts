 ...
 *
 * Returns the authenticated caller's subject, display name, and authentication method.
 */
 ...
 *
 * List all reviews for a specific artifact version, including current and stale markers.
 */
export const listReviewsV0 = <ThrowOnError extends boolean = false>(options: Options<ListReviewsV0Data, ThrowOnError>) => (options.client ?? client).get<ListReviewsV0Responses, ListReviewsV0Errors, ThrowOnError>({ url: '/v0/reviews/{artifactType}/{artifactName}/versions/{version}', ...options });

/**
 * Create an artifact review
 *
 * Record an append-only review for a specific artifact version.
 */
export const createReviewV0 = <ThrowOnError extends boolean = false>(options: Options<CreateReviewV0Data, ThrowOnError>) => (options.client ?? client).post<CreateReviewV0Responses, CreateReviewV0Errors, ThrowOnError>({
...
/**
 * Override a failed artifact review
 *
 * Record an administrative override for a failed review.
 */
export const createReviewOverrideV0 = <ThrowOnError extends boolean = false>(options: Options<CreateReviewOverrideV0Data, ThrowOnError>) => (options.client ?? client).post<CreateReviewOverrideV0Responses, CreateReviewOverrideV0Errors, ThrowOnError>({
...
/**
 * Trigger MCP scoring for a server version
 *
 * Calls the external mcp-scoring service to analyze the server's repository and persists the results.
 */
export const scoreServerV0 = <ThrowOnError extends boolean = false>(options: Options<ScoreServerV0Data, ThrowOnError>) => (options.client ?? client).post<ScoreServerV0Responses, ScoreServerV0Errors, ThrowOnError>({ url: '/v0/servers/{serverName}/versions/{version}/score', ...options });
